...
class DatabaseService {
  ...
  static Future<ChatRoomModel?> getChatRoomModel(UserModel targetUser) async {
    ...
    QuerySnapshot snapshot = await chatsCollection
        ...
        .where("members.${targetUser.uid}", isEqualTo: true)
        .get();
  ...
  static dynamic getChatRooms() {
    dynamic snapshot = chatsCollection
        .where('members.$current_uid', isEqualTo: true)
        .snapshots();
    return snapshot;
  }
  ...
  static savingChatData(
      String chatRoomId, String messageID, MessageModel message) async {
